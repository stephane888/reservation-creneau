/**
 * Derniere version fonctionnelle 16/04/2020.
 */
jQuery(document).ready(function($) {
  const free_horaire = 60;
  // Delai de triatement en jour.
  const free_delai = (window.wbu_free_delai) ? window.wbu_free_delai : 3;
  const plus_delai = (window.wbu_plus_delai) ? window.wbu_plus_delai : 2;
  const express_delai = (window.wbu_express_delai) ? window.wbu_express_delai :
    1;
  //
  ...
  Vue.component('selection_horaire', {
    ...
    methods: {
        ...
        if (this.type == 'livraison') {
          ...
          function waiter() {
            if (!self.update_builder_is_running && !self.builder_is_running) {
              ...
              self.date_select = self.addDays(date_from_recuperation, ( /* not use here : this.date_debut_default + */
              ...
              self.diff_day = self.get_diff_day(self.current_date,
                self.date_select);
              self.builder_update();
              /**
      ...
      get_date_for_old_browser: function(date = null) {
        ...
        if (typeof date === 'string') {
          ...
          if (date_show[2]) {
            ...
            if (ar1[1]) {
              date_show = date_show[0] + '-' + date_show[1] + '-' +
                date_show[2] + 'T' + ar1[1];
            } else {
              date_show = date_show[0] + '-' + date_show[1] + '-' +
                date_show[2];
      ...
      builder_update: function() {
        ...
        execution(1).then(nextExecution, stopExecution).catch(
          function(error) {
            console.log('Error builder catch : ', error);
          });

        function nextExecution(value) {
          execution(value.etape + 1).then(nextExecution,
            stopExecution).catch(function(error) {
            console.log('Error builder catch : ', error);
          });
        }
      ...
      get_select_date: function() {
        ...
        if (this.type == 'recuperation') {
          if (this.builder_is_running && this.maj__date_select != '' &&
            this.valid_default_date() > 0) {
            this.date_select = new Date(this.get_date_for_old_browser(
      ...
      build_tabs: function() {
        ...
        /**
         * on verifie si dans le delai de traitement il ya des jours désactivés.
         */
        if (self.type == 'livraison') {
          ...
          var jour_recuperation = 7 + day_livraison - self.delai_traitement_en_jour;
          if (jour_recuperation > 7) {
            jour_recuperation = jour_recuperation - 7;
          }
          //alert(jour_recuperation + ' :: ' + day_livraison + ' :: ' +
          //  self.delai_traitement_en_jour);
          var jjr = 0;
          var total_jour_delais = Math.abs(parseInt(jour_recuperation)) +
            Math.abs(parseInt(self.delai_traitement_en_jour));
          //var ii = jour_recuperation;

          console.log("total_jour_delais :: ", total_jour_delais,
            jour_desactiver);
          ...
          for (var ii = jour_recuperation; ii <= total_jour_delais; ii++) {
            if (ii <= 6) {
              ...
              w_ii = ii;
              while (($.inArray(w_ii, jour_desactiver) !== -1) &&
                ii <= 14) {
              ...
              w_ii = jjr;
              while (($.inArray(w_ii, jour_desactiver) !== -1) && ii <=
                total_jour_delais) {
                //console.log(' > 6 : ', ii, ' :: ', w_ii);
                jour_supplementaire++;
                if (w_ii > jjr) {
            ...
            console.log("Jour recuperation before  :: ",
              jour_supplementaire);
          }
          /*
          console.log('Jour recuparation', jour_recuperation,
            'delai de traitement', self.delai_traitement_en_jour,
            'jour_supplementaire', jour_supplementaire,
            'total_jour_delais', total_jour_delais);
          /**/
        }
        j = jour_supplementaire;
        this.jour_supplementaire = jour_supplementaire;
        console.log('Jour supplementaire : ' + self.type, this.jour_supplementaire)
        var current_date = self.current_date;
        while (i < self.number_day && j < 7) {
          var date = self.addDays(self.date_select, j); //console.log(' build_tabs  ', self.date_select);
          var day = date.getDay();
          /*
          console.log('Date du jour suivant :', self.type,
            ' :: ', date, 'j index : ', j);
            /**/
           ...
           */
          if ($.inArray(date.getDay(), jour_desactiver) === -1) {
      ...
      getMonthFrench: function(index, type = 'all') {
        var Months = ['Janvier', 'février', 'Mars', 'Avril', 'Mai',
          'Juin', 'Juiellet', 'Aout', 'Septembre', 'Octobre',
          'Novembre', 'Decembre'
        ];
        if (type == 'small') {
      ...
      select_date_tab: function(index) {
        ...
        $.each(this.dates_tabs, function(i, val) {
          if (val.index == index) {
            val.active = true;
            if (index != 'all') {
              self.update__date_select(self.translate_date_to_valid(
                val));
              /**
      ...
      build_creneaux: async function() {
        ...
        return new Promise(async function(resolve, reject) {
          ...
          var plage_all = self.creneau_parjour(self.date_select_utilisable
            .getDay());
      ...
      perfom_build_creneaux: function(plage_all, etape = 'matinee') {
        ...
        return new Promise(async function(resolve, reject) {
          ...
          if (plage_all.matinee) {
            if (etape == 'matinee') {
              if (plage.matinee) {
                await self.buildHoraire(plage.matinee, etape,
                  plage_all);
              }
            } else if (etape == 'soir') {
              if (plage.soir) {
                await self.buildHoraire(plage.soir, etape,
                  plage_all);
              }
            }
          } else {
            if (plage.soir) {
              await self.buildHoraire(plage.soir, 'soir',
                plage_all);
            }
          }
      ...
      buildHoraire: async function(plage, etape_construction, plage_all) {
        ...
        return new Promise(async function(resolve, reject) {
          ...
          var current_date = new Date(self.get_date_for_old_browser(
            self.date_select));
          //var semi=false;
          ...
          function promisecustom(etape = 0, h, mn) {
            ...
            return new Promise((resolve, reject) => {
              ...
              if (self.remove_hours && self.remove_hours[c_hr +
                  '-' + c_mn]) {
                remove_hour = true;
              }
              if (c_hr < 10) {
              ...
              if (parseInt(c_hr) > ht_fin) {
                status = false;
              } else if (parseInt(c_hr) == ht_fin && parseInt(
                  c_mn) > mn_fin) {
                status = false;
              }
              if (status) {
              ...
              function addCrenaux(ct_bl) {
                 ...
                 */
                if (self.builder_is_running && self.perfom__creneau &&
                  self.perfom__creneau.h1) {
                  if (self.perfom__creneau.h1 == ct_bl.h1 &&
          ...
          function successCallback(value) {
            ...
            if ((value.etape + ht_debut + Math.round(self.plage_mn /
                60)) <= ht_fin) {
              if (self.type == 'livraison') {
              ...
              if (mn_debut == 0) {
                if ((value.mn + self.periodicite) < 60) {
                  promisecustom(value.etape, ht_debut, (value.mn +
                    self.periodicite)).then(successCallback,
                    failureCallback);
                } else {
                  promisecustom(value.etape + 1, ht_debut,
                    mn_debut).then(successCallback,
                    failureCallback);
                }
              }
              if (mn_debut != 0) {
      ...
      creneau_parjour: function(index) {
        ...
        if (
          ...
          var hour_now = this.current_date.getHours(),
            hour_final = 0;
          if (plage_valide.matinee && (plage_valide.matinee.ht_fin >=
          ...
          if (plage_valide.soir && (plage_valide.soir.ht_fin >= (
              hour_now + Math.round(this.plage_mn / 60) + this.decalage_heure
            ))) {
            // + decallage
            hour_final = hour_now + this.decalage_heure;
        ...
        if (this.type == 'livraison' && !this.update_builder_is_running) {
          ...
          console.log('\n\n date livraiosn : ', date_livraison,
            ...
            this.delai_traitement_en_jour, 'Jour supplementaire : ',
          ...
          if (this.update__creneau_livraison && this.update__creneau_livraison
            .h1 && this.update__creneau_livraison.h1 != '') {
            ...
            if (plage_valide.matinee) {
              ...
              if (hr_new[0] && (plage_valide.matinee.ht_debut <=
                  hr_new[0]) && (plage_valide.matinee.ht_fin > hr_new[
                  0])) {
            ...
            if (!plage_valide.matinee && plage_valide.soir) {
              ...
              if (hr_new[0] && (plage_valide.soir.ht_debut <= hr_new[
                  0]) && (plage_valide.soir.ht_fin >= hr_new[0])) {
                current_date.setHours(hr_new[0]);
        ...
        if (self.selection_metafields && self.selection_metafields.recuperation &&
          ...
          $.each(self.selection_metafields.recuperation, function(i,
            k) {
            if (!k.mn_debut) {
              k.mn_debut = 0;
            }
      ...
      CalendarMounth: function(days = -25, status = true) {
        ...
        if (days < 0) {
          ...
          while (i >= days) {
            ...
            status = ($.inArray(date.getDay(), jour_desactiver) === -
              1) ? status : false;
            self.months.push({
              status: status,
              date_french: date.getDate().toString().padStart(2,
                '0'),
              date: date.getDate(),
              day: date.getDay(),
              month: date.getMonth(), //date.getMonth().toString().padStart(2, '0'),
              month_french: self.getMonthFrench(date.getMonth(),
                'small'),
              year: date.getFullYear(),
            });
      ...
      select_date_day: function(day) {
        if (day.status) {
          this.dates_tabs = [];
          this.update__date_select(this.translate_date_to_valid(day),
            true);
          this.plage_heure = true;
          this.show_date = false;
          /**
      ...
      valid_date: function() {
        console.log('validation_date ', this.type, ' : ', this.diff_day,
          this.delai_traitement_en_jour,
          ' periode : ', this.periode_day_valide);
      ...
      select_plage_heure: function(index = 0) {
        ...
        var ct_bl = '';
        if (this.default__creneau && this.default__creneau != '' &&
          ...
          index = null;
          //console.log(ct_bl);
        } else if (self.perfom__creneau && self.perfom__creneau != '' &&
          !self.perfom__creneau.h1) {
        ...
        $.each(self.horaires, function(i, hour) {
          if ((ct_bl && ct_bl[0]) &&
            ...
            hour.active = true;
            horaires.push(hour);
            if (self.builder_is_running && self.type ==
              'livraison') {
  ...
  Vue.component('type_livraison', {
    ...
    methods: {
      select_type_tab: function(event, index) {
        var self = this;
        $('.map-localisation-wbu .type-livraison li').removeClass(
          'active');
        ...
        if (this.types_livraison[index]) {
          this.$emit('ev_change_type_livraison', this.types_livraison[
            index]);
        } else {
          console.log('error');
        }
  ...
  window.S_Horaires = new Vue({
    ...
    methods: {
      ...
      date_et_creneau_to_save: function(datas) {
        ...
        texte += " Date : " + date.getDate() + '/' + (date.getMonth() +
          1) + '/' + date.getFullYear() + '\r\n';
        texte += " Heure : " + datas.creneau.h1 + ' - ' + datas.creneau
          .h2 + '\r\n';
      ...
      reload_livraison__date: function(day) {
        ...
        this.date_livraison = day.year + '-' + (day.month + 1) + '-' +
          day.date;
      },
      ...
      selection_init: function() {
        ...
        $('.map-localisation-wbu.container .element-visible').css(
          'display', 'block');
        $('button[name="checkout"], .creneau.open').click(function(
          event) {
          $('.map-localisation-wbu.container .element-visible').css(
            'display', 'block');
      ...
      procced_checkout: async function() {
        ...
        if (this.data_tosave_livraison && this.data_tosave_recuperation) {
          ...
          var wbu_localisation_map = Cookies.get(
            ...
            return new Promise(resolve => {
              ...
              if (self.cart.attributes && self.cart.attributes.livraison &&
                ...
                var livraison = self.cart.attributes.livraison.split(
                  '\r\n');
                var recuperation = self.cart.attributes.recuperation
                  .split('\r\n');
                $.each(recuperation, function(i, val) {
                  texte = val.split(' : ');
                  if (texte[0] && (texte[0].indexOf('Date') !==
                      -1) && texte[1]) {
                    //console.log(texte[1]);
                    var date = texte[1].split('/');
                    self.date_recuperation = date[2] + '-' +
                      date[1] + '-' + date[0];
                  }
                  if (texte[0] && (texte[0].indexOf('Heure') !==
                      -1) && texte[1]) {
                    //console.log(texte[1]);
                    self.default_creneau_recuperation =
                      texte[1];
                  }
                });
                $.each(livraison, function(i, val) {
                  ...
                  if (texte[0] && (texte[0].indexOf('Heure') !==
                      -1) && texte[1]) {
                    //console.log(texte[1]);
                    self.default_creneau_livraison = texte[
                      1];
                  }
                });
            ...
            return new Promise(resolve => {
              ...
              $.each(self.cart.items, function(i, product) {
                if (self.variants[product.id]) {
                   ...
                   */
                  $.each(blocks_type_livraisons, function(k,
                    variant) {
      ...
      updateTypeLivraison: function(datas) {
        ...
        if (new_type_livraison_variant) {
          if (self.variant_in_cart) {
            self.deleteProduct(self.variant_in_cart,
      ...
      get_date_for_old_browser: function(date = null) {
        ...
        if (date_show.isValid()) {
          ...
          if (date_show[2]) {
            date_show[1] = date_show[1].toString().padStart(2, '0');
            date_show[2] = date_show[2].toString().padStart(2, '0');
            return moment(date_show[0] + '-' + date_show[1] + '-' +
              date_show[2]);
          }
        }
        return moment(date);
