jQuery(document).ready(function ($) {
  var blocks_type_livraisons = [{
    titre: 'GRATUIT',
    body: ["Créneau horaire de 120 min", "Délai de traitement de 72 heures", "Minimum de commande : 15,00€"],
    montant: 0,
    type: 'free',
    creneau: 120,
    delai: 3,
    ...
    titre: 'PLUS',
    body: ["Créneau horaire de 60 min", "Délai de traitement de 48 heures", "Minimum de commande : 15,00€"],
    montant: 9.99,
    type: 'plus',
    creneau: 60,
    delai: 2,
    ...
    titre: 'EXPRESS',
    body: ["Créneau horaire de 30 min", "Délai de traitement de 24 heures", "Minimum de commande : 15,00€"],
    montant: 19.99,
    type: 'express',
    creneau: 30,
    delai: 1,
  ...
  Vue.component('selection_horaire', {
    ...
    watch: {
       ...
       */
      perfom__date_select: function perfom__date_select(val) {
        //console.log('Mise à jour de la date de Livraison : ', val);
        this.update_date_livraiosn();
      },
      perfom__creneau: function perfom__creneau(val) {
    ...
    methods: {
        ...
        if (this.type == 'livraison') {
          ...
          var waiter = function waiter() {
            if (!self.update_builder_is_running && !self.builder_is_running) {
              var date_from_recuperation = new Date(self.perfom__date_select);
      ...
      update_creneau_livraiosn: async function update_creneau_livraiosn() {
        /**
      ...
      re_builder: function re_builder() {
        console.log(this.builder_is_running);
        var self = this;
        /**
         * on remet les creneaux à l'initiale
         */
      ...
      builder_update: function builder_update() {
        ...
        function nextExecution(value) {
          execution(value.etape + 1).then(nextExecution, stopExecution).catch(function (error) {
            console.log('Error builder catch : ', error);
          });
          ;
        }
        ...
        function stopExecution(value) {
          ...
          if (!value.etape) console.log('Error on builder stopExecution : ', value);
        }
      },
      display_errors_plage_heure: function display_errors_plage_heure(horaires) {
       ...
       */
      builder: async function builder() {
        var self = this;
        this.builder_is_running = true;
        var etape1 = await self.get_select_date(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01;', etape1, ' type : ', self.type); 

        var etape2 = await self.build_tabs(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01; padding: 2px;', etape2, ' type : ', self.type);

        var etape3 = await self.build_creneaux(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01; padding: 2px;', etape3, ' type : ', self.type);

        var etape4 = await self.buildCalendarMounth(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01; padding: 2px;', etape4, ' type : ', self.type);

        var etape5 = await self.send__date_select(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01; padding: 2px;', etape5, ' type : ', self.type);

        var etape6 = await self.select_plage_heure(); //console.log('%c  Builder fin de etape : %s', 'background: #ccc; color: #1a7b01; padding: 2px;', etape6, ' type : ', self.type);
      ...
      get_select_date: function get_select_date() {
        ...
        if (this.type == 'recuperation') {
          ...
          this.diff_day = this.get_diff_day(this.current_date, this.date_select); //console.log('get_select_date FIN');
          ...
          this.diff_day = this.get_diff_day(this.current_date, this.date_select); //console.log('get_select_date FIN, type ', this.type);
      ...
      build_tabs: function build_tabs() {
        ...
        this.dates_tabs = [];
        var i = 0;
        var current_date = self.current_date; //console.log('Date exacte : ',current_date);
      ...
      get_diff_day: function get_diff_day(date1, date2) {
        var positive = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
      ...
      update__date_select: function update__date_select(date) {
        var rebuild_tab = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
      ...
      build_creneaux: async function build_creneaux() {
        var self = this;
      ...
      perfom_build_creneaux: function perfom_build_creneaux(plage_all) {
        var etape = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'matinee';
        var self = this;
        return new Promise(async function (resolve, reject) {
          var plage = plage_all;
      ...
      buildHoraire: async function buildHoraire(plage, etape_construction, plage_all) {
        var self = this; //console.log('contruction des creneaux de type  ',this.type, ' : ', plage);
        ...
        return new Promise(async function (resolve, reject) {
          ...
          var current_date = new Date(self.date_select); //var semi=false;
          ...
          function promisecustom() {
            var etape = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
            var h = arguments.length > 1 ? arguments[1] : undefined;
            var mn = arguments.length > 2 ? arguments[2] : undefined;
            return new Promise(function (resolve, reject) {
              ...
              function addCrenaux(ct_bl) {
                 ...
                 */
                if (self.builder_is_running && self.perfom__creneau && self.perfom__creneau.h1) {
                  if (self.perfom__creneau.h1 == ct_bl.h1 && self.perfom__creneau.h2 == ct_bl.h2) {
                    ct_bl.active = true;
                  }
                }
          ...
          function successCallback(value) {
            ...
            if (value.etape + ht_debut + Math.round(self.plage_mn / 60) <= ht_fin) {
              if (self.type == 'livraison') {//console.log(' build date : ', (value.etape + ht_debut + Math.round(self.plage_mn/60)), ht_fin,' value : ', value);  
              }
      ...
      creneau_parjour: function creneau_parjour(index) {
        ...
        if (this.type == 'recuperation' && this.diff_day == this.date_debut_default || this.type == 'livraison' && this.diff_day == this.delai_traitement_en_jour + this.date_debut_default) {
        ...
        if (this.type == 'livraison' && !this.update_builder_is_running) {
          ...
          if (this.update__creneau_livraison && this.update__creneau_livraison.h1 && this.update__creneau_livraison.h1 != '') {
      ...
      buildCalendar: function buildCalendar(days, status) {
        var etape = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
      ...
      CalendarMounth: function CalendarMounth() {
        var days = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -25;
        var status = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
        var self = this;
        var date = new Date(this.current_date); //console.log('Current date day ',date.getDate(), 'type ', self.type);
        ...
        if (days < 0) {
          ...
          while (i <= days) {
            ...
            if (self.type == 'recuperation') {
              ...
              if (i <= self.date_debut_default || i <= self.diff_day || i > self.periode_day_valide) {
                status = false;
              }
            }
      ...
      select_plage_heure: function select_plage_heure() {
        ...
        } else if (self.perfom__creneau && self.perfom__creneau != '' && !self.perfom__creneau.h1) {
    ...
    methods: {
      ...
      date_et_creneau_to_save: function date_et_creneau_to_save(datas) {
        ...
        if (this.valid_creneau_recuperation && this.valid_creneau_livraison) {
          ...
          console.log('\n\n Date et creneau à sauvergarder : \n', this.data_tosave_recuperation, this.data_tosave_livraison);
        } else {
          this.valid_creneau = false;
        }
      },
      ...
      reload_livraison__creneau: function reload_livraison__creneau(creneau) {
        ...
        $('button[name="checkout"]').click(function (event) {
          ...
          if (self.get_localisation()) {
            if (self.valid_creneau && self.valid_localisation) {} else {
              event.preventDefault(); //console.log('Paiement trigger');

              var selection = '#id-cart-form';
              self.hidden_block(selection); //console.log("essaie de watch", self.default_type2);
      ...
      procced_checkout: async function procced_checkout() {
        var self = this;
      ...
      analyseCart: function analyseCart() {
        ...
        if (self.cart && self.cart.items) {
          ...
          var execution = async function execution() {
      ...
      save_attribute_cart: function save_attribute_cart(datas) {
        var apply_callback = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
        var self = this;
        return new Promise(function (resolve) {
          self.ajax_watch_attribute = 1;
          jQuery.post(self.url, datas).done(function (data) {
            /**
             * return html
             */
      ...
      */
      deleteProduct: function deleteProduct(id_product) {
        ...
        jQuery.post('/cart/update', product).done(function (data) {
          /**
           * return html
           */
      ...
      hidden_block: function hidden_block(selection) {
        var action = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "continue";
        var self = this;
